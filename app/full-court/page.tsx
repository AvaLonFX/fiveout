...
export default function FullCourtHome() {
  return <main>
    <section className="relative overflow-hidden border-b border-white/10">
      ...
      <div className="relative mx-auto grid min-h-[680px] max-w-7xl items-center gap-14 px-5 py-20 lg:grid-cols-[1.05fr_.95fr]">
        <div>
          ...
          <p className="mt-7 max-w-xl text-lg leading-8 text-slate-300">Cross eras. Draft eight. Set every minute. Then watch your basketball idea survive the court.</p>
    ...
    <section id="how" className="mx-auto max-w-7xl px-5 py-20"><p className="text-xs font-black uppercase tracking-[.3em] text-violet-300">How it works</p><h2 className="mt-3 text-3xl font-black">From zero to tip-off in minutes.</h2><div className="mt-10 grid gap-4 md:grid-cols-3">{features.map(([number,title,copy])=><article key={number} className="rounded-2xl border border-white/10 bg-white/[.035] p-6"><span className="text-sm font-black text-cyan-300">{number}</span><h3 className="mt-8 text-xl font-black">{title}</h3><p className="mt-3 leading-7 text-slate-400">{copy}</p></article>)}</div></section>
  </main>;
}
