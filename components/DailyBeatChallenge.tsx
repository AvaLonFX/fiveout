...
function Leaderboard({ title, subtitle, rows, format }: { title: string; subtitle: string; rows: BoardRow[]; format: (row: BoardRow) => React.ReactNode }) {
  return <div className="rounded-2xl border border-white/10 bg-[#0a1020] p-5">
    ...
    {rows.length ? <ol className="mt-4 space-y-2">{rows.map(row => <li key={`${row.rank}-${row.name}`} className={`grid grid-cols-[2rem_1fr_auto] items-center gap-3 rounded-xl border p-3 ${row.you ? "border-cyan-300/30 bg-cyan-300/[.07]" : "border-white/10 bg-white/[.025]"}`}><span className="text-sm font-black text-slate-500">#{row.rank}</span><span className="min-w-0 truncate font-bold">{row.name}{row.you && <span className="ml-2 rounded-full bg-cyan-300/15 px-2 py-0.5 text-[10px] uppercase text-cyan-200">You</span>}</span><span className="flex items-end gap-2 text-right">{format(row)}</span></li>)}</ol> : <p className="mt-5 rounded-xl border border-dashed border-white/10 p-5 text-center text-sm text-slate-500">No scores yet. Set the first mark.</p>}
  </div>;
}
...
export default function DailyBeatChallenge() {
  ...
  useEffect(() => { const update = () => { const now = new Date(), reset = new Date(now); reset.setUTCHours(24, 0, 0, 0); const seconds = Math.max(0, Math.floor((reset.getTime() - now.getTime()) / 1000)); setCountdown(`${String(Math.floor(seconds / 3600)).padStart(2, "0")}:${String(Math.floor(seconds % 3600 / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`); }; update(); const timer = setInterval(update, 1000); return () => clearInterval(timer); }, []);
  ...
  async function play() {
    ...
    try {
      trackEvent("daily_attempt_started", { attempt: data.attempts.length + 1, budget_used: spent, tactic, seconds_since_open: Math.round((Date.now() - openedAt.current) / 1000) });
      ...
      const latest = body.attempts.at(-1);
      setData(body); trackEvent("daily_attempt_completed", { attempt: body.attempts.length, won: latest?.won ? "yes" : "no", budget_used: spent, margin: latest?.margin });
      if (latest?.won && !data.attempts.some(attempt => attempt.won)) trackEvent("daily_first_win", { attempt: body.attempts.length, margin: latest.margin, budget_used: spent });
      if (body.attempts.length >= body.maxAttempts) trackEvent("daily_challenge_completed", { won_any: body.attempts.some((attempt: Attempt) => attempt.won) ? "yes" : "no", best_margin: Math.max(...body.attempts.map((attempt: Attempt) => attempt.margin)), attempts: body.attempts.length });
  ...
  const nextBadge = [3, 7, 30].find(days => data.streak < days);
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-[1.75rem] border border-violet-400/25 bg-[radial-gradient(circle_at_90%_0%,rgba(139,92,246,.18),transparent_36%),#0a1020]">
      <div className="border-b border-white/10 p-6 sm:p-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.25em] text-violet-300">Daily challenge · {data.day}</p><h1 className="mt-2 text-4xl font-black">Beat this team.</h1><p className="mt-3 max-w-2xl text-slate-400">Build eight players within {data.budget} points. Your first five must cover 2G · 2F · 1C, and every reserve plays at least {data.minBenchMinutes} minutes. You get the same three daily simulations as everyone else.</p>{nextBadge && <p className="mt-3 text-sm font-bold text-violet-200">🏆 {nextBadge-data.streak} more {nextBadge-data.streak === 1 ? "day" : "days"} to unlock the {nextBadge}-day streak badge</p>}</div><div className="grid grid-cols-2 gap-2 text-center text-xs sm:grid-cols-4"><div className="rounded-xl border border-white/10 p-3"><b className="block text-xl text-white">{data.maxAttempts-data.attempts.length}</b>tries left</div><div className="rounded-xl border border-white/10 p-3"><b className="block text-xl text-white">{data.streak}</b>day streak</div><div className="rounded-xl border border-white/10 p-3"><b className="block text-xl text-white">{data.community.beatRate == null ? "—" : `${data.community.beatRate}%`}</b>beat it</div><div className="rounded-xl border border-white/10 p-3"><b className="block text-base text-white">{countdown || "—"}</b>next team</div></div></div></div>
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <div><Leaderboard title="Daily Leaderboard" subtitle="Top 10 today · lower budget breaks a tie" rows={data.leaderboards.daily} format={row => <><b>{row.value > 0 ? `+${row.value}` : row.value}</b><span className="text-xs text-slate-500">{row.score} · {row.spent}/{data.budget}</span></>} />{data.leaderboards.yesterdayWinner && <div className="mt-3 rounded-xl border border-amber-300/20 bg-amber-300/[.06] p-3 text-sm"><span className="text-amber-200">Yesterday’s winner</span><b className="ml-2">{data.leaderboards.yesterdayWinner.name}</b><span className="ml-2 text-slate-400">{data.leaderboards.yesterdayWinner.value > 0 ? "+" : ""}{data.leaderboards.yesterdayWinner.value}</span></div>}</div>
    ...
    {data.attempts.length >= data.maxAttempts && <section className="rounded-2xl border border-white/10 p-6 text-center"><h2 className="text-2xl font-black">Daily complete.</h2><p className="mt-2 text-slate-400">Best result: {best?.score_for}–{best?.score_against}. New opponent in <b className="text-white">{countdown}</b>.</p>{data.signedIn ? <Link href="/full-court/account" className="mt-4 inline-block text-cyan-300 underline">View your Daily badges and history</Link> : <Link href="/full-court/account" className="mt-4 inline-block text-cyan-300 underline">Sign in to keep your streak across devices</Link>}</section>}
  </div>;
}
