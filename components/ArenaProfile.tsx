...
type Profile = { slug: string; displayName: string; isPublic: boolean; own: boolean; stats: { wins: number; losses: number; seriesPlayed: number; games: number; savedMatches: number; pointsFor: number; pointsAgainst: number; winRate: number } };
...
export default function ArenaProfile({ slug }: { slug?: string }) {
  ...
  const shareUrl = typeof window === "undefined" ? "" : `${window.location.origin}/full-court/coach/${profile.slug}`;
  return <main className="mx-auto max-w-4xl py-10 space-y-6">
    <section className="rounded-3xl border border-orange-500/30 bg-gradient-to-br from-orange-500/15 via-card to-card p-8">
      <p className="text-xs font-bold uppercase tracking-[.25em] text-cyan-300">FIVEOUT coach profile</p><h1 className="mt-2 text-4xl font-black">{profile.displayName}</h1><p className="mt-2 text-foreground/60">Your saved match record</p>
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">{[["Saved matches", profile.stats.savedMatches], ["Challenge record", `${profile.stats.wins}–${profile.stats.losses}`], ["Win rate", `${profile.stats.winRate}%`], ["Series games", profile.stats.games]].map(([label, value]) => <div key={label} className="rounded-2xl border bg-background/60 p-4"><p className="text-xs uppercase text-foreground/50">{label}</p><p className="mt-1 text-2xl font-black">{value}</p></div>)}</div>
